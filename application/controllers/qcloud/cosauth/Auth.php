...
class CosAuth {
    ...
    public function createReusableSignature($expiration, $bucket, $filepath = null) {
        ...
        if (empty($appId) || empty($secretId) || empty($secretKey)) {
            $result = self::AUTH_SECRET_ID_KEY_ERROR;
        }else{
            if (empty($filepath)) {
                $result = $this->createSignature($appId, $secretId, $secretKey, $expiration, $bucket, null);
            } else {
                ...
                $result = $this->createSignature($appId, $secretId, $secretKey, $expiration, $bucket, $fileId);
            }
        }        

        if($result == self::AUTH_SECRET_ID_KEY_ERROR){
            $ret['returnCode'] = self::AUTH_SECRET_ID_KEY_ERROR;
            $ret['returnMessage'] = 'Parameter is not valid';
            $ret['returnData'] = '';
        }else{
            $ret['returnCode'] = 0;
            $ret['returnMessage'] = 'OK';
    ...
    public function createNonreusableSignature($bucket, $filepath) {
        ...
        if (empty($appId) || empty($secretId) || empty($secretKey)) {
            ...
            $result = $this->createSignature($appId, $secretId, $secretKey, 0, $bucket, $fileId);
